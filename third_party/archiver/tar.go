...
func (t *Tar) untarNext(to string) error {
	...
	dest, err := safeJoin(to, header.Name)
	if err != nil {
		return err
	}
	return t.untarFile(f, dest)
}
...
func (t *Tar) untarFile(f File, to string) error {
	...
	case tar.TypeSymlink:
		if _, err := sanitizeArchivePath(hdr.Linkname); err != nil {
			return err
		}
		return writeNewSymbolicLink(to, hdr.Linkname)
	case tar.TypeLink:
		linkName, err := sanitizeArchivePath(hdr.Linkname)
		if err != nil {
			return err
		}
		return writeNewHardLink(to, filepath.Join(filepath.Dir(to), filepath.FromSlash(linkName)))
